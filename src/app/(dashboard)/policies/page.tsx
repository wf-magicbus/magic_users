...
export default function PoliciesPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="text-sm font-medium mb-1" style={{ color: "var(--crimson)" }}>Configuration</p>
        <h1 className="text-3xl font-bold tracking-tight" style={{ color: "var(--text-1)" }}>Policies</h1>
        <p className="text-sm mt-1" style={{ color: "var(--text-2)" }}>Group Policy configuration and security settings</p>
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="w-56 flex-shrink-0">
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--surface)", boxShadow: "var(--shadow-md)" }}>
            {navSections.map((section, si) => (
              <div key={section.label} style={si > 0 ? { borderTop: "1px solid var(--border)" } : {}}>
                <div
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: isActive ? "var(--crimson-50)" : "transparent",
                        color: isActive ? "var(--crimson)" : "var(--text-2)",
                        fontWeight: isActive ? 600 : 400,
                        borderRight: isActive ? "2px solid var(--crimson)" : "2px solid transparent",
                      }}
                      onMouseEnter={e => {
                        if (!isActive) {
                    ...
                    >
                      <span style={{ color: isActive ? "var(--crimson)" : "var(--text-3)" }}>{item.icon}</span>
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-5">
            <h2 className="text-lg font-bold" style={{ color: "var(--text-1)" }}>{meta.title}</h2>
            <p className="text-sm mt-0.5" style={{ color: "var(--text-2)" }}>{meta.description}</p>
          </div>
          <ActiveComponent />
        </div>
      </div>
