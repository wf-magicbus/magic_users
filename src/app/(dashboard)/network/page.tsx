...
function PillTabs({ active, onChange }: { active: Tab; onChange: (t: Tab) => void }) {
  return (
    <div className="flex gap-1 p-1 rounded-lg w-fit mb-6 bg-white border border-gray-200 shadow-sm">
      {tabs.map(tab => (
        <button key={tab} onClick={() => onChange(tab)}
          className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${active === tab
...
function Spinner() {
  return (
    <div className="flex items-center gap-2 py-8 text-sm text-gray-500">
      <div className="w-4 h-4 rounded-full border-2 border-t-transparent animate-spin border-yellow-400" style={{ borderTopColor: "transparent" }} />
      Loading…
    </div>
  );
}
...
function TableShell({ headers, children, empty, action }: { headers: string[]; children: React.ReactNode; empty?: boolean; action?: React.ReactNode }) {
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
      {action && <div className="px-6 py-4 flex items-center justify-between border-b border-gray-200">{action}</div>}
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-200 bg-yellow-50">
            {headers.map(h => <th key={h} className="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-700">{h}</th>)}
          </tr>
        </thead>
        <tbody>{children}</tbody>
      </table>
      {empty && <div className="py-12 text-center text-sm text-gray-500">No data found.</div>}
    </div>
  );
}
...
function CrimsonBtn({ onClick, children, small }: { onClick: () => void; children: React.ReactNode; small?: boolean }) {
  return (
    <button onClick={onClick}
      className={`inline-flex items-center gap-1.5 font-semibold text-white rounded-lg bg-yellow-400 hover:bg-yellow-500 transition-colors shadow-sm ${small ? "px-3 py-1.5 text-xs" : "px-4 py-2 text-sm"}`}>
      {children}
    </button>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm text-gray-600">{filtered.length} node{filtered.length !== 1 ? "s" : ""}</span>
        <button onClick={fetch_} className="px-3 py-1 bg-yellow-50 text-yellow-700 text-xs font-semibold rounded hover:bg-yellow-100 border border-yellow-200 transition-colors">
          Refresh
        </button>
      </div>
      <TableShell
            ...
            <tr key={node.id} className="border-b border-gray-100 hover:bg-yellow-50 transition-colors">
              ...
              <td className="px-6 py-4">
                <div className="flex gap-2">
                  <button onClick={() => expire(node.id)} className="px-3 py-1 bg-yellow-50 text-yellow-700 text-xs font-semibold rounded hover:bg-yellow-100 border border-yellow-200 transition-colors">
                    Expire
                  </button>
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#fffefb] to-[#fff8e8]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Headscale VPN infrastructure and node management</p>
          <p className="text-sm mt-1 text-gray-500">
            Headscale VPN — <span className="font-mono text-xs text-gray-500">65.1.54.28:8080</span>
          </p>
        </div>
